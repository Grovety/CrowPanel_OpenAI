...
#define ROTATE_180_ALL_BPP()                                                   \
    {                                                                          \
        ...
        to_index_const = (h - 1) * to_bytes_per_line +                         \
                         (w - x_start - 1) * to_bytes_per_piexl;               \
        for (int from_y = y_start; from_y < y_end + 1; from_y++) {             \
            from_index =                                                       \
                from_y * from_bytes_per_line + x_start * from_bytes_per_piexl; \
...
static void flush_dirty_copy(void *dst, void *src,
    ...
    for (int i = 0; i < dirty_area->inv_p; i++) {
        ...
        if (dirty_area->inv_area_joined[i] == 0) {
            ...
            rotate_copy_pixel((uint8_t *)src, (uint8_t *)dst, x_start, y_start,
                              x_end, y_end, LV_HOR_RES, LV_VER_RES,
...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area,
    ...
    if (lv_disp_flush_is_last(drv)) {
        ...
        if (drv->full_refresh) {
            ...
            rotate_copy_pixel((uint8_t *)color_map, (uint8_t *)next_fb,
            ...
            if (probe_result == FLUSH_PROBE_FULL_COPY) {
                ...
                if (probe_result == FLUSH_PROBE_PART_COPY) {
                     ...
                     * buffer */
                    flush_dirty_save(&dirty_area);
                    flush_dirty_copy(flush_get_next_buf(lcd), color_map,
                                     &dirty_area);
                    flush_get_next_buf(lcd);
                }
            }
...
static lv_disp_t *display_init(LCD *lcd) {
    ...
    ESP_UTILS_CHECK_FALSE_RETURN(lcd->getRefreshPanelHandle() != nullptr,
                                 nullptr, "LCD device is not initialized");
    ...
    for (int i = 0;
         (i < LVGL_PORT_BUFFER_NUM) && (i < LVGL_PORT_BUFFER_NUM_MAX); i++) {
    ...
    for (int i = 0;
         (i < LVGL_PORT_DISP_BUFFER_NUM) && (i < LVGL_PORT_BUFFER_NUM_MAX);
         i++) {
...
static lv_indev_t *indev_init(Touch *tp) {
    ESP_UTILS_CHECK_FALSE_RETURN(tp != nullptr, nullptr,
...
bool lvgl_port_init(LCD *lcd, Touch *tp) {
    ...
    ESP_UTILS_CHECK_FALSE_RETURN(ret == pdPASS, false,
                                 "Create LVGL task failed");
...
bool lvgl_port_lock(int timeout_ms) {
    ESP_UTILS_CHECK_NULL_RETURN(lvgl_mux, false,
                                "LVGL mutex is not initialized");
...
bool lvgl_port_deinit(void) {
    ...
    ESP_UTILS_CHECK_FALSE_RETURN(lvgl_port_unlock(), false,
                                 "Unlock LVGL failed");
