...
static int build_capture_system(void) {
    ...
    esp_capture_audio_afe_src_cfg_t codec_cfg = {
        .record_handle = get_record_handle(),
    };
    capture_sys.aud_src = esp_capture_new_audio_afe_src(&codec_cfg);
...
int test_capture_to_player(void) {
    ...
    esp_capture_setup_path(capture_sys.capture_handle, ESP_CAPTURE_PATH_PRIMARY,
                           &sink_cfg, &capture_path);
    ...
    while ((uint32_t)(esp_timer_get_time() / 1000) < start_time + 10000) {
        ...
        esp_capture_acquire_path_frame(capture_path, &frame, false);
        ESP_LOGI(TAG, "Acquire capture: %lld frame[%u]", esp_timer_get_time(),
                 i++);
